...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on 2nd 'load more' action")
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected two cancelled image URL requests once seconde image is also not visible anymore")
    }
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        loader.completeImageLoading(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url, image.url], "Expected 2nd request when visible after previous completes")
        
        sut.simulateFeedImageViewNotVisible(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url, image.url, image.url], "Expected 3rd request when visible after cancelling previous request")
